...
import it.unipd.dei.db.Utils.InputOutput;
import it.unipd.dei.db.Utils.Lemmatizer;
import it.unipd.dei.db.Utils.WikiPage;
...
{
  public static Dataset<TwitterClustered> exec(Dataset<Twitter> args, SparkSession spark) throws IOException, KcoeffCustomException
  { 
	//***************************************Preprocessing***************************************
    ...
    JavaRDD<Twitter> tweets = args.rdd().toJavaRDD();
    ...
    try
    {
	   ...
	   JavaPairRDD<Integer, ArrayList<Tuple2<Twitter,Vector>>> tuplesToJoin2 = tuplesToJoin.reduceByKey((value1, value2) -> {
		...
		JavaRDD<TwitterClustered> RDDTwitter = newRDD.map((elem) -> {
			TwitterClustered tmp = new TwitterClustered();
			tmp.setTweet_cluster(elem._1);
			tmp.setTweet_vector(elem._2._2);
			tmp.setTweet_ID(elem._2._1.getTweet_ID());
			tmp.setTweet_DATA(elem._2._1.getTweet_DATA());
			tmp.setTweet_text(elem._2._1.getTweet_text());
			tmp.setTweet_URL(elem._2._1.getTweet_URL());
			return tmp;
		});
	    ...
	    for(int f=0; f<k; f++)
	    {
	        ...
	        alist.forEach((elem1) -> {
	           elem1.forEach((elem) -> {
	        	   System.out.println("Twitter Title : " + elem._1().getText());
	           });
	        });
	    } 
